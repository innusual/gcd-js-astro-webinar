...
export const seoContentCollection: SeoCollections = {
  blog: {
    title: "Blog",
    description: "Aquí encontrarás todos los artículos de mi blog",
    entriesPerPage: 10
  },
};
...
export const collectionNames = Object.keys(glob).map((filepath) => path.basename(path.dirname(filepath))).filter((value, index, self) => self.indexOf(value) === index) as ContentCollection[];
